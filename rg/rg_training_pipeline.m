...
%
% Description:
%   Train, validate, and test an RG-LSTM streamflow model using
%   preprocessed CAMELS inputs/targets. The script:
%     - sets paths and loads preprocessed data
%     - configures training and model hyperparameters
%     - prepares dlarray/gpuArray inputs and batching helpers
%     - initializes RG model parameters (LSTM + RG output layer)
%     - runs the training loop with Adam updates, gradient clipping,
%       optional live plotting and logging
%     - selects the best epoch by validation loss
%     - evaluates the best model on the test set, converts outputs to
%       distributional parameters, computes median predictions,
%       back-transforms to original units, and computes per-catchment metrics
%     - saves checkpointed models and summary metrics to a MAT-file
%
% Usage:
%   Place this file in the project folder (or update the paths below),
%   ensure required .mat files exist in the configured data folder,
%   then run the script:
%     rg_training_pipeline
%
...
%   A MAT-file named rg_<nhid>_<epoch>_<miniBatchSize>_<miniBatchFactor>_<seed>.mat
%   containing:
%     - models           : checkpointed parameter structs by epoch
%     - D                : (placeholder; include if defined in workspace)
...
%   camels_metrics
%
% Typical Configuration Variables (set in script):
%   seed, numEpochs, learningRate, miniBatchSize, miniBatchFactor
%   gradientThresholdMethod, gradientThreshold
%   nhid, seq_len, dropoutRate
%   numResponses (fixed = 2), plots, verbose, verboseFrequency
%
% Notes and Assumptions:
%   - Uses dlarray for automatic differentiation and gpuArray when a supported
%     GPU is available. If GPU is present, inputs/targets are converted to GPU.
%   - Random seed is set near the top for reproducibility; gpurng(seed) is used
%     for GPU RNG reproducibility when applicable.
%   - The script expects variables produced by the loaded .mat files:
%     trainIndices, catchmentPartitionInds, inputsScaled, targetScaled,
%     scaleTarget, offsetTarget, nc, etc. Confirm these exist.
%   - Validation/test batching assumes contiguous blocks per catchment as
%     specified in catchmentPartitionInds.
%   - The RG output layer produces two parameters per timestep (mu, sigma).
%     The script computes distributional quantiles (median by default) to
%     produce deterministic point predictions for evaluation.
%   - Saved filename encodes nhid, epoch, miniBatchSize, miniBatchFactor, seed.
%     Confirm 'D' exists in workspace if it should be saved.
%
% Example:
...
addpath F:\projects\Paper_HYDROL74788\hurdle_DL_camels\rg\
...
cd F:\projects\Paper_HYDROL74788\hurdle_DL_camels\rg\
...
% rg output layer
numResponses = 2; % RG distribution has two parameters (mu, sigma) ** do not change
...
parameters = initRG(numResponses, numFeatures, nhid);


%% Training loop

% accelerate estimation of gradients
accfun = dlaccelerate(@gradientsRG);  
...
accfun2 = dlaccelerate(@forwardRG);
...
for ii = 1:numModels
    ...
    for iii = 1:ivBatchesN
        ...
        Pred(:,ivB) = predictionRG(accfun2 ...
            , models(ii).networks, X, dropoutRate);

    end
    
    valLoss(ii) = lossRG(Pred, validTargets_t);
...
for iii = 1:itBatchesN
    ...
    Pred(:,itB) = predictionRG(accfun2, bestParameters, X, dropoutRate);

end

% transform RG predictions into distributional parameters

[mu_t, sigma_t]  = distributionalParametersRG(Pred);

% estimate median of RG distribution (deterministic prediction)
pmed = 0.5; % median (pmed=0.5)
q_t = quantileRG(mu_t, sigma_t, pmed); 

% other quantiles calculated in the same way...
% p99 = 0.99; % use for .99 quantile
% q_t_99 = quantileRG(mu_t, sigma_t, p99); 
